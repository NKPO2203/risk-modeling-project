...
## 6. PROBLÈME DE RECHERCHE — ÉTAT ACTUEL

Le phénomène qui nous intéresse est la concentration du marché actions américain autour d'un nombre limité de très grandes entreprises, dont plusieurs occupent une position importante dans l'écosystème de l'intelligence artificielle.

Nous ne commençons **pas** avec la conclusion « la concentration IA est dangereuse ». La question reste ouverte.

Formulation actuelle, encore modifiable :
...
## 7. BUT OFFICIEL ACTUEL DU PROJET

Version de travail :

> « Ce projet vise à analyser les conséquences de la concentration des portefeuilles dans les grandes entreprises exposées à l'intelligence artificielle sur la performance et le risque de l'investisseur.
>
...
## 12. SÉLECTION DES ENTREPRISES — NE PAS FAIRE AU FEELING

Décision explicite : **ne pas choisir des entreprises parce qu'elles sont célèbres.**

Si nous retenons NVIDIA, Microsoft, Alphabet, Amazon, Meta, etc., il faut une règle documentée.

Critères possibles à étudier :

- poids dans un indice ;
- capitalisation ;
- classification officielle ;
- rôle économique dans l'écosystème IA ;
- exposition IA mesurable et documentée ;
- revenus / capex liés à l'IA ;
- classification issue d'une source institutionnelle ;
- position dans la chaîne de valeur IA : semi-conducteurs, hyperscalers, cloud, software, data centers.

Rien n'est encore choisi. Il faudra aussi traiter correctement les entreprises à **plusieurs classes d'actions** (Alphabet A/C, etc.) : l'indice compte les classes séparément, ce qui gonfle mécaniquement le nombre de lignes sans ajouter d'entreprise.

**⚠ Contrainte nouvelle et impérative — voir §44 et §45 :** la règle de sélection ne doit jamais s'appuyer sur la performance boursière passée, et ne peut pas se réduire à un secteur GICS.
...
**État réel au 4 septembre 2026 :** seuls existent `README.md`, `main.py`, `risk_analysis.ipynb` et le dossier `research/` (créé, contenant ce document). Le reste est l'architecture **cible**.
...
Le vrai code du projet **n'a pas commencé.** Ne saute pas à `yfinance`.

Avant le code : Research Charter, définitions, question de recherche, hypothèses, règles de sélection, sources, méthodologie générale. **Ensuite** seulement : données, code.
...
**Corollaire acquis :** aucun texte n'est validé parce qu'il est bien écrit — ni celui d'un assistant, ni le mien. Tout ce qui entre dans le charter passe les trois tests du §42.
...
Fait à vérifier sur la source primaire mais bien établi : dans la classification GICS,
...
**Erreur à éviter :** chercher une seule date pour trois usages différents. Il en faut trois, justifiées séparément.

| Période | Rôle | Contrainte |
|---|---|---|
| **Observation du phénomène** | montrer l'évolution de la concentration | peut être longue (contexte historique) |
| **Estimation** | calculer volatilités, corrélations, VaR | stabilité statistique : trop longue mélange les régimes, trop courte produit du bruit |
| **Périodes de stress** | épisodes courts et datés, examinés individuellement (ch. 9) | choisies pour leur nature, pas pour leurs résultats |

**Les deux façons de tricher :**

*Par le haut* — commencer en 2023 : on démarre l'échantillon au moment où le phénomène se produit, donc on est certain de le trouver (sélection sur le résultat), et l'échantillon ne contient presque aucun épisode de stress sérieux → **impossible de tester l'hypothèse C**.

*Par le bas* — remonter à 1995 pour inclure la bulle internet : composition d'indice différente, entreprises différentes, régime de taux différent, et le terme « IA » y est anachronique. On compare deux mondes en prétendant qu'ils forment un seul échantillon.

**Critère de décision :** une date de départ n'est défendable que si elle est justifiée par quelque chose d'**extérieur au résultat cherché** — (a) une rupture structurelle documentée par une source, (b) une contrainte de disponibilité des données, (c) la nécessité de couvrir un nombre suffisant d'épisodes de stress.

**Proposition de travail (non verrouillée) :**

- **Observation du phénomène : 2015 → 2026** (~11 ans), justifiée par (c) : l'intervalle contient plusieurs régimes distincts — correction de fin 2018, krach Covid de février-mars 2020, marché baissier et choc de taux de 2022, turbulences d'avril 2025 liées aux tarifs douaniers, et la correction de 2026 liée au conflit au Moyen-Orient. Matière suffisante pour l'hypothèse C, sans avoir choisi la période pour ses résultats.
- **Le 30 novembre 2022** (mise à disposition publique de ChatGPT) sert de **point de rupture** découpant l'échantillon en deux sous-périodes comparables — **pas** de date de départ. Comme date de départ, c'est de la sélection sur le résultat ; comme point de rupture, c'est un test de changement de régime, méthodologiquement propre.
- **Robustesse** : refaire les calculs clés sur des fenêtres alternatives. Si les conclusions s'effondrent en décalant la fenêtre de deux ans, cela doit figurer dans le rapport, pas être caché.
...
*Raison :* le S&P 500 n'est pas retenu parce qu'il est populaire, mais parce qu'il constitue **le portefeuille de référence de l'investisseur diversifié** — l'indice que détient concrètement quelqu'un qui pense ne prendre aucun pari. Le phénomène n'a d'intérêt que s'il affecte quelqu'un, et c'est cet investisseur-là qu'il affecte.
*Alternatives écartées :*
- **Nasdaq-100** — plus concentré en technologie, le phénomène y serait plus spectaculaire, mais personne ne le détient en croyant être diversifié : ce serait enfoncer une porte ouverte.
...
> ⚠ **STATUT : PROPOSITION NON VALIDÉE.** Ce squelette a été rédigé par l'assistant, pas par l'auteur, et n'a été approuvé par personne. Il ne peut pas être clos tant que la question ouverte du §47 (lecture (i) vs (ii)) n'est pas tranchée. Cinq points y restent flous : la valeur de N ; le choix de la mesure de concentration ; la quantification de « plusieurs de ces entreprises » ; l'absence de point de comparaison historique ; et l'affirmation « réputé diversifié », non sourcée.
...
- **Note de bas de page 9** (p. 10) : l'exposition des ménages américains aux actions représente environ 30 % de leurs actifs totaux, en grande partie via des véhicules passifs indexés sur le S&P 500 (401(k), ETF). **→ Appui direct pour l'Observation 3 et pour la justification D-001.**
...
**Étape en cours :** Research Charter, bloc 1 — *Phenomenon of Interest*.

**Acquis dans cette session :**
- distinction bloc 1 / bloc 2 / bloc 3 comprise ;
- trois tests du phénomène établis (§39) ;
- critère phénomène / résultat acquis — vérifié par exercice de classement, 5/6, seule erreur : le max drawdown d'un portefeuille construit, qui est un résultat et non un fait ;
- piège de circularité « AI-related » identifié et paré (§41) ;
- piège de la classification GICS identifié (§42) ;
- traitement des périodes clarifié : trois périodes distinctes, proposition 2015-2026, rupture au 30 novembre 2022 (§43) ;
- indice de référence choisi provisoirement : S&P 500, avec alternatives écartées et motifs (§44) ;
- squelette du bloc 1 rédigé, en attente de chiffrage (§45) ;
- première source primaire vérifiée et exploitée : FMI GFSR avril 2026 (§46).

**NON acquis — le bloc 1 n'est ni rédigé ni validé.** Quatre tentatives de formulation ont échoué au même endroit, et la cause n'était pas la rédaction (voir décision bloquante ci-dessous). N'ont pas non plus été tranchés : la valeur de N, la mesure de concentration retenue, la période, et aucun chiffre n'a été collecté.

---

### ⛔ DÉCISION BLOQUANTE — à trancher avant toute autre chose

Le projet oscille entre **deux lectures incompatibles**, jamais départagées :

| | **Lecture (i)** | **Lecture (ii)** |
|---|---|---|
| Objet étudié | la **concentration en méga-capitalisations** du S&P 500 | un **portefeuille d'entreprises IA** |
| Rôle de l'IA | explication candidate de *pourquoi* la concentration s'est produite | **critère de sélection** des titres |
| Rôle du S&P 500 | l'objet lui-même | simple benchmark |
| Portefeuilles construits | l'indice, et des variantes dé-concentrées | un panier IA vs l'indice |

Ce sont **deux projets distincts** : entreprises différentes, chapitres différents, conclusions différentes. Tant que ce n'est pas tranché, aucune phrase du bloc 1 ne peut tenir.

**Recommandation de l'assistant : lecture (i).** Motifs : (1) elle est mesurable sans avoir à définir « entreprise IA » au préalable, donc elle échappe au piège de circularité du §41 — en (ii), il faudrait résoudre le problème de définition *avant* de pouvoir énoncer le phénomène, ordre impossible ; (2) elle correspond au §35, l'idée la plus forte du projet ; (3) elle ne retire rien des phases B et C.
**Prix à payer, à voir avant de choisir :** en (i), l'IA cesse d'être le sujet et devient l'explication candidate. Le centre de gravité du titre change.

**Statut : ouvert. Décision de l'auteur, pas de l'assistant.**

---

**Ensuite, dans l'ordre, une fois la décision prise :**
1. trancher N et la mesure de concentration ;
2. trancher la période d'observation ;
3. chiffrer les observations du §45 sur sources primaires ;
4. écrire `research/research_charter.md` — bloc 1 ;
5. bloc 2 — *Research Problem* ;
6. bloc 3 — *Research Question* ;
7. hypothèses formalisées, critères de sélection, sources, méthodologie.

**Autres questions ouvertes :**
- définition opérationnelle de « AI-related » : construire la nôtre, ou partir d'un univers institutionnel (M7 du FMI ? Bloomberg AI index ?) puis l'auditer ?
- indice principal définitivement verrouillé sur S&P 500 ?
- périodes d'estimation et de stress à fixer ;
- benchmarks définitifs ;
- traitement des classes d'actions multiples.

---

### Méthode de travail retenue

Deux assistants en parallèle. Règle : **le dépôt fait foi** — ce qui n'est pas écrit dans `research/` n'est pas décidé. Les assistants proposent ; le fichier tranche.

Répartition : l'autre assistant pour apprendre, explorer et produire les premiers jets ; celui-ci pour **décider, auditer et vérifier les sources primaires**. Venir avec un texte à attaquer, jamais avec une page blanche.

**Ce qui n'a pas commencé :** le code du projet, la sélection définitive des entreprises, la collecte de données, le rapport LaTeX.
